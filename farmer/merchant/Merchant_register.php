...
if ($_SERVER['REQUEST_METHOD'] =='POST') {
    if ($_POST['password'] == $_POST['confirmpassword']) {
        ...
        $result = $mysqli->query("SELECT * FROM musers WHERE mail='$email'") or die($mysqli->error);
        if (preg_match("!image!",$_FILES['avatar']['type'])) {
            if ( $result->num_rows > 0 ) {
                ...
                if(copy($_FILES['avatar']['tmp_name'], $avatar_path)) {
                    ...
                    $sql = " INSERT INTO musers(username, fullname, mail, mobile, password, hash, avatar) "." VALUES ('$username', '$fullname', '$email', '$mobile', '$password', '$hash', '$avatar_path') ";
                    if ( $mysqli->query($sql) ){
        ...
        <div id="container">
                ...
                <div id="content">
                    <ul>
                        ...
                        <li style="float:right"><a class="actives" href="Merchant_login.php">Merchant Login</a></li>
                        <li style="float:right"><a class="actives" href="../farmer/Farmer_login.php">Farmer Login</a></li>
                      </ul>   
                        
                        <form class = "signup" action="Merchant_register.php" enctype="multipart/form-data" method="POST">
                                <h3 class="signup-title">Merchant Register</h3 class="signup-title">
                                ...
                                <a class="login" href="Merchant_login.html"> Already Register? </a > 
                        </form>
                        </div>
                </div>
               <div id="footer">
